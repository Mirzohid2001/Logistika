...
const AdvancedAnalyticsScreen = () => {
  ...
  const [analyticsType, _setAnalyticsType] = useState<'driver' | 'client'>(
    user?.is_driver ? 'driver' : 'client'
  );
  ...
  const renderEarningsAnalysis = () => {
    if (!analytics?.earnings_analysis) {
      return <SectionEmptyNote title={t('analytics.earningsAnalysis')} />;
    }

    const { earnings_analysis } = analytics;

    return (
      <Card variant="soft" style={styles.card}>
        <Text style={styles.cardTitle}>{t('analytics.earningsAnalysis')}</Text>
        <View style={styles.statsRow}>
          <View style={styles.statItem}>
            <Text style={styles.statLabel}>{t('analytics.totalEarnings')}</Text>
            <Text style={styles.statValue}>{formatPrice(earnings_analysis.total_earnings)}</Text>
          </View>
          <View style={styles.statItem}>
            <Text style={styles.statLabel}>{t('analytics.totalPayments')}</Text>
            <Text style={styles.statValue}>{earnings_analysis.total_payments}</Text>
          </View>
        </View>
        <View style={styles.statsRow}>
          <View style={styles.statItem}>
            <Text style={styles.statLabel}>{t('analytics.averagePerOrder')}</Text>
            <Text style={styles.statValue}>{formatPrice(earnings_analysis.average_per_order)}</Text>
          </View>
        </View>
        {earnings_analysis.daily_earnings.length > 0 && (
          <View style={styles.chartContainer}>
            <Text style={styles.chartTitle}>{t('analytics.dailyEarnings')}</Text>
            <View style={styles.barChart}>
              {earnings_analysis.daily_earnings.map((item, index) => {
                const maxEarnings = Math.max(...earnings_analysis.daily_earnings.map(e => e.earnings));
        ...
        {analytics.best_routes.map((route, index) => (
          <View key={index} style={styles.routeItem}>
            ...
            <View style={styles.routeStats}>
              ...
              {(route.total_earnings || route.total_cost) && (
                <Text style={styles.routeAmount}>
                  {formatPrice(route.total_earnings || route.total_cost || 0)}
                </Text>
              )}
            </View>
          </View>
  ...
  return (
    ...
    >
      <View style={styles.content}>
        <AppHeader variant="hero" title={t('analytics.title')} />
        {analyticsType === 'driver' ? renderEarningsAnalysis() : renderExpensesAnalysis()}
        {renderBestTimes()}
        {renderBestRoutes()}
      </View>
    </ScrollView>
