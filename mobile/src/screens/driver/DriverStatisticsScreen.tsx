...
const DriverStatisticsScreen = () => {
  ...
  const loadStatistics = useCallback(async () => {
    try {
      setLoading(true);
      const data = await ordersService.getDriverStatistics();
      setStatistics(data);
    } catch (error) {
  ...
  const maxDailyEarnings = Math.max(...statistics.daily_earnings.map((d) => d.earnings), 1);
  const maxMonthlyEarnings = Math.max(...statistics.monthly_earnings.map((m) => m.earnings), 1);
  ...
  return (
    <ScreenBackground>
      <ScrollView
        ...
        <AppHeader variant="hero" title={t('statistics.title')} subtitle={t('statistics.earnings')} />
        <Card variant="elevated" style={styles.summaryCard}>
          <Text style={styles.summaryTitle}>{t('statistics.totalEarnings')}</Text>
          <Text style={styles.summaryAmount}>{formatMoney(statistics.total_earnings)}</Text>
        </Card>

        <Card variant="soft" style={styles.statsCard}>
          <Text style={styles.cardTitle}>{t('statistics.earnings')}</Text>
          <View style={styles.statRow}>
            <View style={styles.statItem}>
              <Text style={styles.statLabel}>{t('statistics.today')}</Text>
              <Text style={styles.statValue}>{formatMoney(statistics.earnings_today)}</Text>
            </View>
            <View style={styles.statDivider} />
            <View style={styles.statItem}>
              <Text style={styles.statLabel}>{t('statistics.thisWeek')}</Text>
              <Text style={styles.statValue}>{formatMoney(statistics.earnings_week)}</Text>
            </View>
            <View style={styles.statDivider} />
            <View style={styles.statItem}>
              <Text style={styles.statLabel}>{t('statistics.thisMonth')}</Text>
              <Text style={styles.statValue}>{formatMoney(statistics.earnings_month)}</Text>
            </View>
          </View>
        </Card>
        ...
        <Card variant="soft" style={styles.statsCard}>
          ...
          <View style={styles.statRow}>
            ...
            <View style={styles.statDivider} />
            <View style={styles.statItem}>
              <Text style={styles.statLabel}>{t('orders.pending')}</Text>
            ...
            <View style={styles.statDivider} />
            <View style={styles.statItem}>
              <Text style={styles.statLabel}>{t('orders.inProgress')}</Text>
          ...
          <View style={styles.statItem}>
            ...
            <Text style={styles.statValue}>{formatMoney(statistics.avg_order_amount)}</Text>
          </View>
        </Card>

        {statistics.daily_earnings.length > 0 && (
          <Card variant="soft" style={styles.statsCard}>
            <Text style={styles.cardTitle}>{t('statistics.dailyEarnings7')}</Text>
            <View style={styles.chartContainer}>
              {statistics.daily_earnings.map((item, index) => {
                const date = new Date(item.date);
                const dayName = date.toLocaleDateString(currentLanguage === 'ru' ? 'ru-RU' : 'uz-UZ', {
                  weekday: 'short',
                });
                const height = maxDailyEarnings > 0 ? (item.earnings / maxDailyEarnings) * 150 : 0;

                return (
                  <View key={index} style={styles.barContainer}>
                    <View style={[styles.bar, { height: Math.max(height, 5) }]} />
                    <Text style={styles.barLabel}>{dayName}</Text>
                    <Text style={styles.barValue}>{(item.earnings / 1000).toFixed(0)}k</Text>
                  </View>
                );
              })}
            </View>
        ...
        {statistics.monthly_earnings.length > 0 && (
          <Card variant="soft" style={styles.statsCard}>
            <Text style={styles.cardTitle}>{t('statistics.monthlyEarnings6')}</Text>
            <View style={styles.chartContainer}>
              {statistics.monthly_earnings.map((item, index) => {
                ...
                const label =
                  monthIndex >= 0
                    ...
                    : item.month.split('-')[1];
                const height = maxMonthlyEarnings > 0 ? (item.earnings / maxMonthlyEarnings) * 150 : 0;

                return (
                  <View key={index} style={styles.barContainer}>
                    <View style={[styles.bar, { height: Math.max(height, 5) }]} />
                    <Text style={styles.barLabel}>{label}</Text>
                    <Text style={styles.barValue}>{(item.earnings / 1000000).toFixed(1)}M</Text>
                  </View>
                );
              })}
            </View>
...
export default DriverStatisticsScreen;
