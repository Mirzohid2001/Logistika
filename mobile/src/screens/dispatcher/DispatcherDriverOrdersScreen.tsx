...
const DispatcherDriverOrdersScreen = () => {
  ...
  const loadOrders = useCallback(async () => {
    try {
      setLoading(true);
      const data = await dispatcherService.getDriverOrders(driverId);
      setOrders(data);
    } catch (error) {
  ...
  return (
    <ScreenBackground>
      <AppHeader variant="hero" title={t('dispatcherLists.driverOrdersTitle')} />
      {loading ? (
        <SkeletonCard />
      ) : (
        <FlatList
...
export default DispatcherDriverOrdersScreen;
